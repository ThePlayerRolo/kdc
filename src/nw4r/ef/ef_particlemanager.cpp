...
int ParticleManager::RetireParticleAll() {
    ...
    for (pIt = static_cast<Particle*>(mActivityList.mActiveList.first);
         pIt != NULL; pIt = pNext) {

        pNext = static_cast<Particle*>(
            NW4R_UT_LIST_GET_LINK(mActivityList.mActiveList, pIt)->next);
...
void ParticleManager::BeginCalc(bool onlyIfRemain) {
    ...
    Particle* pIt =
        static_cast<Particle*>(mActivityList.mActiveList.first);
